...
const HeaderMain = () => {
  ...
  const { photo, cartItems } = useGallery();
  const photoFavoriteCount =
    photo.length > 0
      ? photo.filter((item) => item.isFavorite === true).length
      : 0;

  const cartCount = cartItems.length > 0 ? cartItems.length : 0;
  return (
    <div className="p-4 bg-white shadow-md flex items-center justify-center">
      ...
      <div className="ml-auto mr-5 flex items-center gap-5">
        <span className="relative w-8">
          ...
          <span className="w-5 h-5 rounded-full flex items-center justify-center text-white text-xs bg-red-500 absolute top-0 right-0 translate-x-2/4 ">
            {cartCount}
          </span>
        </span>
