...
const Courses = ({ youtubeItems }) => {
  return (
    <div className="youtube-item">
      <div className="youtube-image">
        <img src={youtubeItems.image} alt="" className="" />
      </div>
      <div className="youtube-footer">
        <img src={youtubeItems.avatar} alt="" className="youtube-avatar" />
        <div className="youtube-info">
          <h3 className="youtube-title">{youtubeItems.title}</h3>
          <h4 className="youtube-author">{youtubeItems.author}</h4>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="youtube-list">
      {youtubeItems.map((course) => {
        <Courses key={youtubeItems.id} youtubeItems={course} />;
      })}
    </div>
  );
}
ReactDOM.render(<AppEx />, document.getElementById("root"));
export default App;
